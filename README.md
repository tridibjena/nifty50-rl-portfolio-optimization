...
**Full numbers: [RESULTS.md](RESULTS.md)** — generated by the pipeline, never edited by
hand. **Plan of record: [ROADMAP.md](ROADMAP.md)**.
...
python3 scripts/run_pipeline.py     # data → regimes → walk-forward → figures → RESULTS.md
...
scripts/          run_pipeline.py · render_report.py (figures + report, no retraining)
tests/            79 tests; every bug fix has one that fails against the original
